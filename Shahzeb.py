...
This property makes `Meershahzeb-` useful for peer-to-peer applications. `ssb-db` also makes it easy to encrypt 
messages.
...
Building upon `Meershahzeb-` requires understanding a few concepts that it uses to ensure the unforgeability of 
message feeds.
...
Note that `Meershahzeb ` does not include facilities for retrieving a blob given the hash.

## Replication

It is possible to easily replicate data between two instances of `Meershahzeb`.
...
`Meershahzeb` maintains useful security properties even when it is connected to a malicious ssb-db database. This 
...
`Meershahzeb` adds persistence to a [SecretStack](https://github.com/ssbc/secret-stack) setup.
Without other plugins, this instance will not have replication or querying. Loading `Meershahzeb ` directly is 
...
hooked by a number of plugins, but `Meershahzeb` adds an `indexes` section (which represents how fully built the 
indexes are).
...
Return the version of `Meershahzeb `. currently, this returns only the ssb-db version and not the ssb-server version, 
